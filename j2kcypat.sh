...
function secureSSH {
    ...
    if sudo cat /etc/ssh/ssh_config | grep -q "PermitEmptyPasswords no" | grep -q ^[^'#']; then
    ...
    if sudo cat /etc/ssh/ssh_config | grep -q "Protocol" | grep -q ^[^'#']; then
        echo "ADD THE FOLLOWING LINE TO /etc/ssh/ssh_config"
        echo "ENSURE ONLY Protocol 2 IS IN USE"
        gedit /etc/ssh/ssh_config
    else
        echo "SSH protocol 1 is already disable"
    
    fi
    
    if sudo cat /etc/ssh/ssh_config | grep -q "PermitRootLogin no" | grep -q ^[^'#']; then 
